...
def show_records():
    ...
    dates = [{'filled_date':record.filled_date ,"end date":(datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).date()-timedelta(days=10)),"Start date":datetime.strptime("2024-04-23", '%Y-%m-%d').date()} for record in records]
    return dates    
#Show all record
...
def get_records():
    ...
    try:
        ...
        # end_date = datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).date()+ timedelta(days=10)
    try:
        # Parse the date string into a date object
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d').date()
    except ValueError:
...
def get_auto_code_2(db,**kwargs):
    ...
    today_str = datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).strftime('%y%m%d')
...
def add_record():
    ...
    interaction_content = data.get('interaction_content')
    interaction_result = data.get('interaction_result')
    filled_date = datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).strftime("%Y-%m-%d %H:%M:%S")
    # assistant = ''
...
def edit_record(code):
    ...
    new_record_history = Customer_Record_History(filled_date = current_record_data["filled_date"],
                                                  created_at = datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).strftime("%Y-%m-%d %H:%M:%S"), 
                                                  code= code, 
    ...
    record.recent_changed_at = datetime.now(timezone(timedelta(hours=+2), 'Helsinki')).strftime("%Y-%m-%d %H:%M:%S")
